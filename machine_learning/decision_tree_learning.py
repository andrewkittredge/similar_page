...
from preprocessing_utilities import prep_e_tree_for_parsing
...
def decision_tree(data_set, attributes, parent_node):
    ...
    if all(example['class'] == data_set[0]['class'] for example in data_set):
        ET.SubElement(parent_node, 'leaf_node' , {'class' : data_set[0]['class']})
    elif not attributes:
        most_frequent_class = plurality_value(data_set)
        ET.SubElement(parent_node, 'leaf_node', {'class' : most_frequent_class})
    else:
        ...
        if impurity_reduction(best_attribute) < THRESHOLD:
            ET.SubElement(parent_node, 'leaf_node', {'class' : plurality_value(data_set)})
        else:
            ...
            for attribute_value, subset in disjoint_subsets:
                branch_node = ET.SubElement(node, 'branch_node', {'attribute_value' : attribute_value})
...
class Tester(unittest.TestCase):
    ...
    def test_matches_model(self):
        ...
        root_node = ET.Element('root')
        decision_tree(self.data_set, self.data_set.attributes, root_node)
        
        no_applicant = {'age' : 'y',
                     'has_job' : False,
                     'own_house' : False,
                     'f' : False}
        self.assertFalse(matches_model(root_node, no_applicant))
        ...
        self.assertTrue(matches_model(root_node, yes_applicant))
